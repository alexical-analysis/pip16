...
async fn main() {
    ...
    loop {
        ...
        // step the vm multiple times per frame
        for i in 0..256_000 {
            if i % 64 == 0 {
                vm.test();
            }
        ...
        draw_text(&format!("{:.0} fps", get_fps()), 5.0, 15.0, 16.0, WHITE);
        next_frame().await;
...
fn load_gfx_script(vm: &mut VM) {
    vm.load(&[
        EncInst::new_addi(1, 0, 16).into(), // 00: r1 = r0(0) + 16
        ...
        EncInst::new_addi(6, 0, 2).into(),  // 13: r6 = r0(0) + 2
        EncInst::new_sw(1, 3, 0).into(),    // 14: mem[r3(SPR_X_POS) + 0] = r1(20)
        EncInst::new_sw(2, 4, 0).into(),    // 15: mem[r4(SPR_Y_POS) + 0] = r2(30)
        EncInst::new_sw(6, 5, 0).into(),    // 16: mem[r5(PPU_CONTROL) + 0] = r6(2) [draw sprite]
        EncInst::new_addi(1, 0, 1).into(),  // 17: r1 = r0(0) + 1
        EncInst::new_addi(2, 0, 1).into(),  // 18: r2 = r0(0) + 1
        EncInst::new_addi(3, 0, 1).into(),  // 19: r3 = r0(0) + 1
        EncInst::new_lw(4, 0, 7).into(),    // 20: r4 = mem[r0(0) + 7] (HALT_CONTROL)
        EncInst::new_sw(4, 3, 0).into(),    // 21: mem[r4(HALT_CONTROL) + 0] = r3(1)
        EncInst::new_addi(3, 0, 9).into(),  // 22: r3 = r0(0) + 9
        EncInst::new_jalr(0, 3).into(),     // 23: jump to addr in r3 (9)
    ]);
}
